    ...
    <header>
      <div class="first-header">
        ...
        <b-nav class="navbar">
          ...
          <b-nav-item href="http://localhost:8080/companies"
            >Compañias</b-nav-item
          >
          <b-nav-item @click="logOut()" href="http://localhost:8080"
            >Salir</b-nav-item
          >
        </b-nav>
      </div>
...
export default {
  ...
  methods: {
    ...
    showMsgBoxOne() {
      this.boxOne = "";
      this.$bvModal
        .msgBoxConfirm("¿Está seguro que desea eliminar los registros?")
        .then((value) => {
          this.boxOne = value;
          ...
          value === true && isSelected > 0
            ...
            ? this.$bvModal.msgBoxOk(
                "Debe seleccionar al menos 1 registro para eliminar"
              )
            : this.cancel();
        })
        .catch((err) => {
    ...
    validateNewEmail(oldEmail, newEmail) {
      ...
      const duplicatedEmailMap = duplicatedEmail
        .map((e) => e.email)
        .find((e) => e === newEmail);
    ...
    async createEmployee() {
      ...
      if (!formatDataValidate && !duplicatedEmail) {
        ...
        duplicatedEmail
          ? this.$bvModal.msgBoxOk(
              `El email "${this.employeeSelected.email}" ya se encuentra registrado`
            )
          : this.$bvModal.msgBoxOk(
              "Alguno de los formatos requeridos no son válidos o existen campos vacíos"
            );
      }
    },
    ...
    async updateEmployee() {
      const formatDataValidate = this.disableSubmit();
      if (
        this.validateNewEmail(this.employees, this.employeeSelected.email) ==
        true
      ) {
        this.$bvModal.msgBoxOk(
          `El email "${this.employeeSelected.email}" ya se encuentra registrado`
